...
use App\Http\Controllers\NegociacoesController;
...
Route::middleware(['auth','auth.admin'])->name('dashboard.')->group(function() {
    ...
    Route::resource('/negociacao', NegociacoesController::class)->except(['create', 'store']);
    ...
    Route::post('/user/estado', [UserController::class, 'userEstado'])->name('user-estado');
    Route::post('/user/cidade', [UserController::class, 'userCidade'])->name('user-cidade');
    Route::post('/user/corretores', [UserController::class, 'bairroCorretor'])->name('user-corretor');
});
